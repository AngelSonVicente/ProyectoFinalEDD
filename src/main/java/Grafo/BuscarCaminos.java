...
public class BuscarCaminos {
    ...
    public List<Camino> encontrarCaminos(Nodo origen, Nodo destino, float horaActual) {
        ...
        encontrarCaminosRecursivo(origen, destino, new ArrayList<>(), new ArrayList<>(), 0, 0, 0, horaActual, 0, caminos);
        return caminos;
    }

    private void encontrarCaminosRecursivo(Nodo actual, Nodo destino, List<Nodo> caminoActual, List<Arista> aristasCamino, float rapidezActual, int Gasolina, int distanciaActual, float horaActual, int tiempoVehiculo, List<Camino> caminos) {
            ...
            for (Arista arista : aristas) {
                ...
                if (arista.getInicio().equals(actual)) {
                    ...
                    if (!caminoActual.contains(siguiente)) {
                        encontrarCaminosRecursivo(siguiente, destino, caminoActual,nuevasAristasCamino, rapidezArista + rapidezActual, Gasolina + consumoGasArista, distanciaActual + distanciaArista, util.sumarMinutosAHora(horaActual, tiempoArista), tiempoVehiculo + tiempoArista, caminos);
                    }
